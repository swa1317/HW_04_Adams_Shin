...
def BirdbathFunc423( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.94601, -0.74162, -0.77094 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.15619,  0.67005, -0.52571 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.28402,  0.03219, -0.35958 ] )
